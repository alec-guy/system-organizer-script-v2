...
physicallyMakeFolder folder = do 
          ...
          case folder of 
            (Folder set) -> do
                ...
                sequence_  $ (\oldPath -> cp (Text.unpack oldPath) (newPath oldPath)) <$> files 
                            
physicallyMakeFolders :: [Folder] -> IO ()
physicallyMakeFolders folders = 
    Prelude.foldr (\folder acc -> (physicallyMakeFolder folder) >> acc) (pure ()) folders

makeFolderName :: Folder -> String 
makeFolderName folder = 
    case folder of 
        (Folder set) -> case elemAt 0 set of 
                         elem -> (Data.Char.toUpper <$> extension' elem) ++ ("-folder") 
